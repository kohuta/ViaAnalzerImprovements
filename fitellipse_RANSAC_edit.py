...
import random
import cv2
import configparser #used to create INI files that can be edited by user, not necessarily useful
#---------------------------------------------
...
def Geometric2Conic(ellipse):
    ...
    a, b = max([aa/2, bb/2]),min([aa/2,bb/2])
...
def Circularity(pnts,ellipse):
    ...
    #std of rotated coordinants
    stheta=[]
    for phi in np.linspace(0,np.pi):
        stheta.append(np.sqrt(((np.cos(phi)**2)*sum(X**2)+(np.sin(phi)**2)*sum(Y**2) + np.sin(2*phi)*sum(X*Y))/len(X)))
    return min(stheta)/max(stheta)


#    #calculate the angle for each of the pts
#    t = np.arctan((pts[:,1]-y0)/(pts[:,0]-x0))
#    
#    #return x,y coordinants of model for each angle
#    coords= [x0 + aa*np.cos(t), y0 + bb*np.sin(t)]
#    diff = []
#    for p,c in zip(pnts,coords):
#        diff.append(np.sqrt((p[1]-c[1])**2+(p[0]-c[0])**2))
#   
#    return np.std(diff)
...
def shapeData(dat):
    return dat.reshape(len(dat),2)   
 
if __name__ == "__main__":
    #run to generate contours
    os.system('python ./Test.py')
    
    test = cnt[185] #chose decent sized fit contour from resulting data

    cfg = configparser.ConfigParser()
    cfg['OUTPUT'] = {'graphics':0}
    
    #reshape data to Nx2 array
    pts = shapeData(test)
    
    #run RANSAC 
    best_ellipse,inliers = FitEllipse_RANSAC(pts,img,cfg,max_itts=10, max_refines=20,max_perc_inliers=90.0)
    
    print(Circularity(pts,best_ellipse))
    
    best_ellipse,inliers = FitEllipse_RANSAC_Support(pts,gray_blurred,cfg,max_itts=10, max_refines=20,max_perc_inliers=90.0)
    print(Circularity(pts,best_ellipse))
    
    best_ellipse,inliers = FitEllipse_RobustLSQ(pts,img,cfg,max_its = 10, max_refines=10,max_perc_inliers=90)
    print(Circularity(pts,best_ellipse))
